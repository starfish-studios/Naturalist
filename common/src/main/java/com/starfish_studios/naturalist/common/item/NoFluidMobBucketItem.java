...
public class NoFluidMobBucketItem extends MobBucketItem {
    public NoFluidMobBucketItem(EntityType<? extends Mob> entitySupplier, Fluid fluid, SoundEvent emptyingSound, Properties settings) {
    ...
    public InteractionResult use(Level pLevel, Player pPlayer, InteractionHand pHand) {
        ...
        if (blockhitresult.getType() == HitResult.Type.MISS) {
            ...
            if (pLevel.mayInteract(pPlayer, pos) && pPlayer.mayUseItemAt(blockpos1, direction, itemstack)) {
                ...
                return InteractionResult.SUCCESS.heldItemTransformedTo(getEmptySuccessItem(itemstack, pPlayer));
            } else {
