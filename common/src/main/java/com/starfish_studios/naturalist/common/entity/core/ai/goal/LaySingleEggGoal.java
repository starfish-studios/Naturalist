...
public class LaySingleEggGoal<T extends Animal & EggLayingAnimal> extends MoveToBlockGoal {
    ...
    public LaySingleEggGoal(T animal, double speedModifier) {
    ...
    public void tick() {
        ...
        if (!this.animal.isInWater() && this.isReachedTarget()) {
            if (this.animal.getLayEggCounter() < 1) {
                ...
                level.setBlock(this.blockPos.above(), this.animal.getEggBlock().defaultBlockState().setValue(TurtleEggBlock.EGGS, this.animal.getRandom().nextInt(1) + 1), 3);
