...
public class NaturalistEntityAttributes {
    ...
    /*public static void registerFabric() {
        ENTRIES.forEach(entry -> FabricDefaultAttributeRegistry.register(entry.type().get(), entry.builder().get()));
    }
    *///?}

    //? if forge {
    public static void registerForge(EntityAttributeCreationEvent event) {
        ENTRIES.forEach(entry -> event.put(entry.type().get(), entry.builder().get().build()));
    }
    //?}
}
